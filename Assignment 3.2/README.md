...
Simulation Parameters : To verify the theoretical outcomes of the bridge rectifier, the following parameters were configured in LTspice:

 -AC Voltage Source ($V_{in}$): 100V Peak Amplitude, 50 Hz Frequency.                  -Stop Time: 40ms                                                                      -Components: 4 Diodes (Bridge configuration), Load Resistor ($R_L$), and Filter Capacitor ($C$).
